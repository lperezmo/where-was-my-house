...
import { ATTRIBUTION, fetchGeology } from "./_lib/macrostrat";
import {
  clamp,
  getQuery,
  isReadMethod,
...
export default {
  async fetch(request: Request): Promise<Response> {
    try {
      ...
      const result = await fetchGeology({ lat, lon, maxMa, minMa });
      const response = sendJson(200, result, CACHE_CONTROL);
      // GeologyResult has no field for it, but CC-BY needs the credit to travel
      // with the data.
      ...
      return sendError(500, "Could not load geologic units");
    }
  },
};
